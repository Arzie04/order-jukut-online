/**
 * BACKUP - Google Apps Script untuk Order Jukut Online (CURRENT VERSION WITH ERRORS)
 * Script ini harus di-copy paste langsung ke Google Apps Script yang terhubung dengan spreadsheet
 * 
 * Cara setup:
...
function doPost(e) {
  try {
    ...
    // endpoint untuk update stock
    if (body.action === "order") {
      return orderStock(body);
    }

    // endpoint untuk update stock (array)
...
function getOrders() {
  ...
  if (!sheet) {
    return jsonOutput({ error: `Sheet "${ORDERS_SHEET_NAME}" tidak ditemukan` });
  }
...
function getConfig() {
  ...
  try {
    // Ambil konfigurasi dari kolom F2:H2
