...
/* =========================
   TEST FUNCTION untuk validasi item count
========================= */
function testItemCountLogic() {
  // Test dengan contoh data user: PKT PA 2, NP DD 3, EXT TG 3
  // Expected result: 5 (hanya PKT dan NP yang dihitung)
  
  const testPesanan = "PKT PA 2\nNP DD 3\nEXT TG 3";

  const lines = testPesanan.split('\n');
  let totalItemCount = 0;
    ...
    if (parts.length >= 2) {
      ...
      if (prefix === 'PKT' || prefix === 'NP') {
        if (parts.length >= 3) {
          const qty = parseInt(parts[2], 10) || 1;
          totalItemCount += qty;
          console.log(`${prefix} ${parts[1]} ${qty} - ditambahkan ke count`);
        } else {
          // Jika tidak ada angka, anggap 1
          totalItemCount += 1;
          console.log(`${prefix} ${parts[1]} 1 - ditambahkan ke count`);
        }
      } else {
        console.log(`${prefix} ${parts[1]} ${parts[2] || 1} - TIDAK dihitung (EXT)`);
      }
    }
  });
  
  console.log(`Total item count: ${totalItemCount}`);
  console.log("Expected: 5 (PKT PA 2 + NP DD 3 = 5)");
  console.log("Test ${totalItemCount === 5 ? 'PASSED' : 'FAILED'}");
